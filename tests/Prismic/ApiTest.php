...
class ApiTest extends TestCase
{
    ...
    {
        return Api::get('https://whatever.prismic.io/api/v2', 'My-Access-Token', $this->httpClient->reveal(), $this->cache->reveal(), 99);
    }
    protected function getApiWithDefaultData()
    {
    ...
    public function testGetIsCalledOnHttpClientWhenTheCacheIsEmpty()
    {
        ...
        $this->cache->set(Argument::type('string'), Argument::type('string'), 99)->shouldBeCalled();
        $api = $this->getApi();
    ...
    {
        return [[['io.prismic.preview' => 'preview', 'other' => 'other'], 'preview'], [['io.prismic.preview' => 'preview', 'io.prismic.experiment' => 'experiment'], 'preview'], [['io_prismic_preview' => 'preview'], 'preview']];
    }
    /**
     * @dataProvider getPreviewRefs
     */
    public function testPreviewRefIsReturnedWhenPresentInSuperGlobal(array $cookie, $expect)
    {
        $_COOKIE = $cookie;
        $api = $this->getApiWithDefaultData();
    ...
    public function testCorrectExperimentRefIsReturnedWhenCookieIsSet()
    {
        ...
        $expectedRef = 'VDUUmHIKAZQKk9uq';
        // The ref at index 1 for the variations in this experiment
    ...
    {
        $runningGoogleCookie = '_UQtin7EQAOH5M34RQq6Dg 1';
        $_COOKIE = ['io.prismic.experiment' => $runningGoogleCookie, 'io.prismic.preview' => 'Preview Ref Cookie Value'];
        $api = $this->getApiWithDefaultData();
        $this->assertTrue($api->inPreview());
        $this->assertFalse($api->inExperiment());
    }

 ...
 */
const PRISMIC_CONFIG = ['api' => 'https://your-repository-name.cdn.prismic.io/api/v2', 'token' => null];
...
class Finder
{
    ...
    public function body()
    {
        ...
        $title = sprintf('Viewing Document ID# <code>%s</code> of type “%s”', $this->document->id, $this->document->type);
    ...
    private function recentDocs($count = 10)
    {
        /** @var SearchForm $form */
        $form = $this->api->forms()->everything;
        $form = $form->ref($this->api->ref());
    ...
    private function listRecentDocs($count = 10)
    {
        $markup = [];
        ...
        foreach ($this->recentDocs($count) as $link) {
            $markup[] = sprintf('<a class="list-group-item list-group-item-action" href="/?id=%1$s">ID: %1$s, Type: %2$s</a>', $link->id, $link->type);
        }
        $markup[] = '</div>';
        return implode("\n", $markup);
    }

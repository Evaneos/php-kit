...
class Ref
{
     ...
     */
    private function __construct($id, $ref, $label, $isMasterRef, $maybeScheduledAt = null)
    {
        $this->id = $id;
        $this->ref = $ref;
        $this->label = $label;
    ...
    public function getScheduledDate()
    {
        ...
        return DateTimeImmutable::createFromFormat('U', (string) $this->getScheduledAtTimestamp());
    }
    /**
    ...
    public static function parse(stdClass $json)
    {
        if (!isset($json->id, $json->ref, $json->label)) {
            throw new Exception\InvalidArgumentException('The properties id, ref and label should exist in the JSON object for a Ref');
        }
        return new Ref($json->id, $json->ref, $json->label, isset($json->{'isMasterRef'}) ? $json->isMasterRef : false, isset($json->{'scheduledAt'}) ? $json->scheduledAt : null);
    }
}
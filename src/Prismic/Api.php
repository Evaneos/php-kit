...
class Api
{
     ...
     */
    private $httpClient;
    private function __construct(ApiData $data, $accessToken = null, ClientInterface $httpClient = null, CacheInterface $cache = null)
    {
        $this->data = $data;
     ...
     * @return self
     */
    public static function get($action, $accessToken = null, ClientInterface $httpClient = null, CacheInterface $cache = null, $apiCacheTTL = 5)
    {
     ...
     */
    public function getRefFromLabel($label)
    {
        $refs = $this->refs();
        return $refs[$label];
    }
     ...
     */
    public function previewSession($token, LinkResolver $linkResolver, $defaultUrl)
    {
        try {
        ...
        if (isset($response->mainDocument)) {
            $documents = $this->query(Predicates::at("document.id", $response->mainDocument), ['ref' => $token, 'lang' => '*'])->results;
    ...
    {
        $cookieNames = [str_replace(['.', ' '], '_', self::PREVIEW_COOKIE), self::PREVIEW_COOKIE];
    ...
    {
        $cookieNames = [str_replace(['.', ' '], '_', self::EXPERIMENTS_COOKIE), self::EXPERIMENTS_COOKIE];
        foreach ($cookieNames as $cookieName) {
            if (isset($_COOKIE[$cookieName])) {
     ...
     * @return stdClass|null the resulting document (null if no match)
     */
    public function getByUID($type, $uid, array $options = [])
    {
        $options = $this->prepareDefaultQueryOptions($options);
     ...
     * @return stdClass|null    the resulting document (null if no match)
     */
    public function getSingle($type, array $options = [])
    {
     ...
     * @return array
     */
    private function prepareDefaultQueryOptions(array $options)
    {

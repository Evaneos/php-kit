...
class SearchFormTest extends TestCase
{
    ...
    {
        return new SearchForm($this->httpClient->reveal(), $this->cache->reveal(), $this->form, $this->form->defaultData());
    }
    ...
    private function assertScalarOptionIsSet(SearchForm $form, $key, $expectedValue)
    {
        $data = $form->getData();
        $this->assertArrayHasKey($key, $data);
        $this->assertSame($expectedValue, $data[$key]);
    }
    private function assertScalarOptionIsNotSet(SearchForm $form, $key)
    {
        $data = $form->getData();
    ...
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->after('Whatever'), 'after', 'Whatever');
    }
    public function testLang()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->lang('en-gb'), 'lang', 'en-gb');
    }
    public function testPageSize()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->pageSize(99), 'pageSize', 99);
    }
    public function testPage()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->page(99), 'page', 99);
    }
    public function testFetchWithStringArgs()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->fetch('one', 'two', 'three'), 'fetch', 'one,two,three');
    }
    ...
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->fetchLinks('one', 'two', 'three'), 'fetchLinks', 'one,two,three');
    }
    public function testGraphQueryWithStringArg()
    {
        ...
        $this->assertScalarOptionIsSet($this->getSearchForm()->graphQuery($query), 'graphQuery', $query);
    }
    public function testOrderingsWithStringArgs()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->orderings('one', 'two', 'three'), 'orderings', '[one,two,three]');
    }
    public function testOrderingsStripsSquareBrackets()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->orderings('[my.foo desc]', '[my.bar]'), 'orderings', '[my.foo desc,my.bar]');
    }
    public function testOrderingsWillAcceptUnpackedArrays()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->orderings(...['[my.a]', 'my.b', 'my.c desc']), 'orderings', '[my.a,my.b,my.c desc]');
    }
    public function testOrderingsFiltersEmptyValues()
    {
        $this->assertScalarOptionIsSet($this->getSearchForm()->orderings(...['', 'my.b', '', 'my.c desc']), 'orderings', '[my.b,my.c desc]');
    }
    public function testOrderingsIsNotSetWhenOnlyEmptyValuesAreProvided()
    {
        $this->assertScalarOptionIsNotSet($this->getSearchForm()->orderings(...['', '']), 'orderings');
    }
    ...
    {
        $query = [Predicates::at('document.id', 'SomeId'), Predicates::any('document.tags', 'Some Tag')];
    ...
    public function testExceptionIsThrownForInvalidForm()
    {
        ...
        $searchForm = new SearchForm($this->httpClient->reveal(), $this->cache->reveal(), $form, $form->defaultData());
        $searchForm->submit();
    }
    ...
    private function prepareResponse($body = null)
    {
        $body = $body ? $body : '{"data":"data"}';
        $response = new Response(200, ['Cache-Control' => 'max-age=999'], $body);
    ...
    public function testResponseJsonIsReturned()
    {
        ...
        $this->cache->get(Argument::type('string'))->willReturn(null);
        $this->cache->set(Argument::type('string'), Argument::type(\stdClass::class), 999)->shouldBeCalled();
        $form = $this->getSearchForm();

...
class RequestFailureException extends RuntimeException
{
     ...
     */
    protected static function fromGuzzleRequestException(RequestException $e)
    {
        ...
        $url = $request->getUri();
        $message = sprintf('The %s request to the repository %s resulted in a %d %s error. Complete URL: %s', $request->getMethod(), $url->getHost(), $code, $reason, (string) $url);
